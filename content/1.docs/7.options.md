...
You can set the paths to the files used by the emulator. This is useful if you want to host the files in a different way than the default, like a blob or a cdn.
...
A unique numeric ID for the game. Used to keep saves, save states, and cached files separate between games. It is also required for netplay.

- Type: `number`
- Default: `1`
- Example: `EJS_gameID = 3`

### `EJS_cheatPath`

Base path or URL the cheat manager loads cheat files from. The emulator looks for a global `cheats.json` and a per-system `{system}.json` at this location.

- Type: `string`
- Default:&nbsp;` `
- Example: `EJS_cheatPath = 'https://my.website/cheats/'`
...
Forces the use of the legacy (WebGL 1) cores even if the browser supports WebGL 2.

- Type: `boolean`
- Default: `false`
- Example: `EJS_forceLegacyCores = true`
...
An array of setting IDs to hide from the settings menu.

- Type: `array`
- Default: `[]`
- Example: `EJS_hideSettings = ['shader', 'fps']`

### `EJS_browserMode`

Forces the emulator to use the mobile or desktop UI instead of auto-detecting. Accepts `1` or `"mobile"` to force mobile, and `2` or `"desktop"` to force desktop. Any other value (or leaving it unset) auto-detects.

- Type: `number` or `string`
- Default:&nbsp;` ` (auto-detect)
- Example: `EJS_browserMode = 'mobile'`

### `EJS_askBeforeExit`

Whether to show a confirmation prompt before exiting the emulator when the exit button is pressed. Set to `false` to exit immediately without confirmation.

- Type: `boolean`
- Default: `true`
- Example: `EJS_askBeforeExit = false`
...
If set, this will force the core to flush the entire save file to disk every `EJS_fixedSaveInterval` milliseconds,
...
too low will dramatically increase resource usage for the client.

- Type: `number`
- Default: `` (unset)
- Example: `EJS_fixedSaveInterval = 7000` (Save every 7 seconds)

### `EJS_netplayServer`

URL to the netplay server. Required to enable netplay. Note that netplay also requires `EJS_gameID` to be set to a number.

- Type: `string`
- Default:&nbsp;` `
- Example: `EJS_netplayServer = 'https://netplay.emulatorjs.org/'`

### `EJS_netplayICEServers`

An array of ICE (STUN/TURN) servers used for the WebRTC netplay connection.
...
Adds custom shaders, merged with the built-in shaders. Each key is the shader name and the value is the shader source. These become selectable in the shader settings menu.
...
Disables saving and loading the emulator settings to the browser's `localStorage`.

- Type: `boolean`
- Default: `false`
- Example: `EJS_disableLocalStorage = true`

### `EJS_disableAutoUnload`

By default the core is unloaded from memory when the emulator exits. Set this to `true` to keep it loaded after exit.

- Type: `boolean`
- Default: `false`
- Example: `EJS_disableAutoUnload = true`

### `EJS_disableBatchBootup`

Disables the batched bootup process used by the DOS core.

- Type: `boolean`
- Default: `false`
- Example: `EJS_disableBatchBootup = true`

### `EJS_noAutoFocus`

Prevents the emulator from automatically focusing its container element. Useful if auto-focus interferes with the rest of your page.

- Type: `boolean`
- Default: `false`
- Example: `EJS_noAutoFocus = true`
...
Function to be called when the save SAV files button is pressed. If this handler is set, it overrides the default behaviour (downloading the save file).
...
```
- Example: `EJS_onSaveSave = function(e) {console.log("save files button pressed!")}`

### `EJS_onLoadSave`

Function to be called when the load SAV files button is pressed. If this handler is set, it overrides the default behaviour (opening the file picker).

- Type: `function`
- Default:&nbsp;` `
- Example: `EJS_onLoadSave = function() {console.log("load files button pressed!")}`

### `EJS_onExit`

Function to be called when the emulator exits.

- Type: `function`
- Default:&nbsp;` `
- Example: `EJS_onExit = function() {console.log("The emulator has exited!")}`
...
An array of translation strings that were requested but not found in the loaded language file. This is used for debugging and adding new languages. It is exposed on the emulator instance, so you can read it with `console.log(EJS_emulator.missingLang)` after the emulator has started.

- Type: `array`
- Default: `[]`
- Example: `console.log(EJS_emulator.missingLang);`
